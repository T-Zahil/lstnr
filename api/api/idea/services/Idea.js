...
module.exports = {
  ...
  add: async (values) => {
    ...
    const entry = await Idea.create(data);
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Idea.associations.map(a => a.alias));
    const data = _.omit(values, Idea.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Idea.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Idea.associations.map(async association => {
