  ...
  findOne: async function ( params, populate ) {
    ...
    if ( _.isEmpty( populate ) ) {
      const arrayOfPromises = this.associations
        ...
        .map( association => { // eslintdisableline nounusedvars
        ...
        switch ( association.nature ) {
          ...
          case 'oneToOne':
            if ( response[ current ] !== params.values[ current ] ) {
              ...
              virtualFields.push(
                strapi.query( details.model || details.collection, details.plugin ).findOne( {
                  id: recordId
                } )
                .then( record => {
          ...
          case 'manyMorphToOne':
            ...
            params.values[ current ].forEach( obj => {
              const model = obj.source && obj.source !== 'contentmanager' ?
